...
SELECT "Years", SUM("Rice_production") AS total_rice_production_top3
FROM "AgriData_Explorer"
...
WITH year_range AS (
  SELECT MAX("Years") AS latest_year
  FROM "AgriData_Explorer"
),
yield_change AS (
  SELECT "Distname",
         MAX(CASE WHEN "Years" = (SELECT latest_year FROM year_range) THEN "Wheat_yield" END) AS recent_yield,
         MAX(CASE WHEN "Years" = (SELECT latest_year FROM year_range) - 4 THEN "Wheat_yield" END) AS old_yield
  FROM "AgriData_Explorer"
  GROUP BY "Distname"
)
...
WITH growth AS (
  SELECT "Statename",
         SUM(CASE WHEN "Years" = (SELECT MAX("Years") FROM "AgriData_Explorer") THEN "Oilseeds_production" END) AS recent,
         SUM(CASE WHEN "Years" = (SELECT MAX("Years") FROM "AgriData_Explorer") - 4 THEN "Oilseeds_production" END) AS past
  FROM "AgriData_Explorer"
  GROUP BY "Statename"
)
SELECT "Statename",
...
SELECT "Years", "Statename", SUM("Cotton_production") AS cotton_production
FROM "AgriData_Explorer"
WHERE "Statename" IN (SELECT "Statename" FROM top_states)
GROUP BY "Years", "Statename"
ORDER BY "Years", "Statename";

6.Districts with the Highest Groundnut Production in 2017 (till 2017)

SELECT DISTINCT "Years" FROM "AgriData_Explorer" ORDER BY "Years";
...
SELECT "Years", ROUND(AVG("Maize_yield")::numeric, 2) AS avg_maize_yield
FROM "AgriData_Explorer"
GROUP BY "Years"
ORDER BY "Years";
...
WITH top_states AS (
  ...
  FROM "AgriData_Explorer"
  WHERE "Years" >= EXTRACT(YEAR FROM CURRENT_DATE) - 10
  GROUP BY "Statename"
  ORDER BY total_prod DESC
  LIMIT 5
),
combined AS (
  SELECT "Years", "Statename", 'Rice' AS crop, SUM("Rice_production") AS production
  FROM "AgriData_Explorer"
  WHERE "Statename" IN (SELECT "Statename" FROM top_states)
    AND "Years" >= EXTRACT(YEAR FROM CURRENT_DATE) - 10
  GROUP BY "Years", "Statename"
  UNION ALL
  SELECT "Years", "Statename", 'Wheat' AS crop, SUM("Wheat_production")
  FROM "AgriData_Explorer"
  WHERE "Statename" IN (SELECT "Statename" FROM top_states)
    AND "Years" >= EXTRACT(YEAR FROM CURRENT_DATE) - 10
  GROUP BY "Years", "Statename"
